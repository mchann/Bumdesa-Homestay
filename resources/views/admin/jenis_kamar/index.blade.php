...
@section('title', 'Kelola Jenis Kamar')
...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            ...
            <p class="text-gray-500 mt-1">Total: {{ count($jenisKamar) }} jenis kamar</p>
        </div>
        <a 
            href="{{ route('admin.jenis-kamar.create') }}" 
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden border border-gray-100">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $item->nama_jenis }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm space-x-2">
                            <!-- DELETE (tetap sama, gunakan jenis_kamar_id) -->
                            <form 
                                action="{{ route('admin.jenis-kamar.destroy', $item->jenis_kamar_id) }}" 
                                method="POST" 
                                class="inline"
                                onsubmit="return confirm('Yakin hapus {{ $item->nama_jenis }}?')"
                            >
                                @csrf 
                                @method('DELETE')
                                <button 
        ...
        <div class="px-6 py-3 bg-gray-50 text-right text-xs text-gray-500">
            Menampilkan 1 sampai {{ count($jenisKamar) }} dari {{ count($jenisKamar) }} hasil
        </div>
    </div>
</div>
