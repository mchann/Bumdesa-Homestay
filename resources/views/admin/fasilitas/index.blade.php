...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            ...
            <p class="text-gray-500 mt-1">Total: {{ count($fasilitas) }} fasilitas</p>
        </div>
        <a 
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden border border-gray-100">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $item->nama_fasilitas }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm space-x-2">
                            <!-- DELETE -->
                            <form 
                                action="{{ route('admin.fasilitas.destroy', $item->fasilitas_id) }}" 
                                method="POST" 
                                class="inline"
                                onsubmit="return confirm('Yakin hapus {{ $item->nama_fasilitas }}?')"
                            >
                                @csrf 
                                @method('DELETE')
                                <button 
        ...
        <div class="px-6 py-3 bg-gray-50 text-right text-xs text-gray-500">
            Menampilkan 1 sampai {{ count($fasilitas) }} dari {{ count($fasilitas) }} hasil
        </div>
    </div>
</div>
