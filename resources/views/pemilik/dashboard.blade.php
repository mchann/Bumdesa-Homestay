...
<div class="stats-grid">
    ...
    <div class="stat-card">
        ...
        <div class="stat-change positive">
            ...
            <span>{{ $kamarBaruBulanIni ?? 0 }} kamar ditambahkan bulan ini</span>
        </div>
    </div>
    ...
    <div class="stat-card">
        ...
        <div class="stat-change positive">
            ...
            <span>
                @if(($totalKamar ?? 0) > 0)
                    {{ round(($kamarTersedia / $totalKamar) * 100, 1) }}% tersedia
                @else
                    0% tersedia
                @endif
            </span>
    ...
    <div class="stat-card">
        ...
        <div class="stat-value">{{ $totalPemesanan ?? 0 }}</div>
        <div class="stat-change positive">
            <i class="fas fa-arrow-up"></i>
            <span>{{ $persenPemesananBulanIni ?? 0 }}% dari bulan lalu</span>
        </div>
    </div>
    ...
    <div class="stat-card">
        ...
        <div class="stat-change positive">
            <i class="fas fa-arrow-up"></i>
            <span>{{ $persenPendapatanBulanIni ?? 0 }}% dari bulan lalu</span>
        </div>
    </div>
</div>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($pemesananTerbaru as $p)
                            <tr>
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm font-medium text-gray-900">{{ $p->pelanggan->name ?? '-' }}</div>
                                    <div class="text-xs text-gray-500">{{ $p->pelanggan->email ?? '-' }}</div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-800">{{ $p->kamar->nama_kamar ?? '-' }}</div>
                                    <div class="text-xs text-gray-500">{{ $p->kamar->homestay->nama_homestay ?? '-' }}</div>
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-800">
                                    {{ date('d-m-Y', strtotime($p->tgl_check_in)) }}
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-800">
                                    {{ date('d-m-Y', strtotime($p->tgl_check_out)) }}
                                </td>
                                <td class="px-6 py-4">
                                    @php
                                        $statusClasses = [
                                    ...
                                    @endphp
                                    <span class="px-3 py-1 rounded-full text-xs font-medium {{ $statusClasses[$p->status] ?? 'bg-gray-50 text-gray-700' }}">
    ...
    const ordersChart = new Chart(ordersCtx, {
        ...
        data: {
            ...
            datasets: [
                {
                    label: 'Berhasil',
                    data: @json(array_values($dataBulananLengkap['berhasil'])),
                    borderColor: '#10B981',
                ...
                {
                    label: 'Pending',
                    data: @json(array_values($dataBulananLengkap['pending'])),
                    borderColor: '#F59E0B', 
                ...
                {
                    label: 'Dibatalkan',
                    data: @json(array_values($dataBulananLengkap['dibatalkan'])),
                    borderColor: '#EF4444',
    ...
    const statusData = @json($statusPemesanan);
